...
%======================================================================
function [data,data2]=CalculaModeloNS_ScriptEGG(data,plt,wref, MUTFP, REFFP, ExtractExp)
% ANTES SE LLAMABA: logarithmicCocientAJG3editedWR_wo1day_data_example(data,plt,wref, MUTFP, REFFP, ExtractExp);
...
for p = 1:length(plt);%p = 1:length(plt);
    ...
    LimiteMUT_FP = max(pl.mut(:,MUTFP)); %limit of the mutant's Fluorescence Protein
    LimiteREF_FP = max(pl.ref(:,REFFP));
...
%jaar    f=find(ismember(np_OUTG,[2:3])); % esto creo que es muy especifico de garay, podria ser que aqui es donde hay que dejar solo los puntos de fase exponencial
    ...
    for tmpw=1:length(fwu);
        ...
        if sum(isnan(LOGRC(ftmp)))>100; %2*4dias %NUMERO Mágico! cuantos NaNs estás dispuesto a tolerar para que un pozo se siga procesando 
            remw=[remw;welli];
        end;
    end;
    ...
    dataplt2=dataplt1(f,:);
%    save PL1 dataplt2 %ESTO SE GUARDABA ANTES, AHORA YA NO.
    %Falta depurar filas donde no todos los valores son NaN
    [Aplt,Splt,Gplt,Cplt,ICS,ICG,res2,wells2,days2]=LinearRegresionMatrizData(dataplt2,[wref MUTFP REFFP]); 
